...
# 这段代码实现了一个基于扩散模型的图像生成训练框架，主要包含教师-学生模型训练、隐式模型优化和对抗训练三个核心部分。
class TrainLoop:
    def __init__(
            self,
            *,
        ...
        self.teacher_model, self.student_model, self.implicit_model = self.accelerator.prepare(self.teacher_model,
                                                                                               self.student_model,
                                                                                               self.implicit_model)
        self.data, self.opt_d, self.opt_s, self.criterion, self.opt_disc = self.accelerator.prepare(self.data,
                                                                                                    self.opt_d,
                                                                                                    self.opt_s,
                                                                                                    self.criterion,
                                                                                                    self.opt_disc)
    ...
    def get_measurements(self, clean):
        ...
        if self.step % 100 == 0:
            vtils.save_image(blur, os.path.join(self.save_dir, 'train_measurement', f'step{self.step}_y.png'),
                             normalize=True)
    ...
    def forward_implicit(self, clean, blur, noise=None):
        ...
        for i in range(0, blur.shape[0], self.micro_batch):
            ...
            with self.accelerator.autocast():
                ...
                # else: 
                disc_weight = self.calculate_adaptive_weight(gt_consistency_loss, gan_loss,
                                                             self.implicit_model.module.output_blocks[-1][0].out_layers[
                                                                 -1].weight)
        ...
        if self.use_wandb and self.accelerator.is_main_process:
            wandb.log({'KL loss': kl_loss.item(), 'Con loss': consistency_loss.mean().item(), 'Total loss': loss.item(),
                       'Gan loss': gan_loss.item(), 'disc weight': disc_weight.item()}, step=self.step)
    ...
    def get_psnr_lpips(self, clean, recon, train=False, start_idx=0):
        ...
        for i in range(b):
            psnr = psnr_loss((clean[i].cpu().numpy() + 1.) / 2.,
                             (recon[i].detach().cpu().numpy() + 1.) / 2.)  # psnr data range [0.1]
            lpips_loss = self.lpips(clean[i].reshape(1, c, h, w), recon[i].reshape(1, c, h,
                                                                                   w)).item()  # lpips range [-1.1] with mean [0,0,0] and std [1,1,1]
            psnr_list.append(psnr)
            lpips_list.append(lpips_loss)
            logger.log('steps %d: %s  %dth image PSNR: %.3f / LPIPS: %.4f' % (
                self.step, print_script, start_idx + i, psnr, lpips_loss))
